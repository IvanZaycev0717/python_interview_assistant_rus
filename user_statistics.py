...
def get_right_answers_amount(progress: dict) -> StatInformation:
    ...
    basic_progress = get_paticular_progress(progress, basic_amount, qt.BASIC_FIRST_QUESTION, qt.BASIC_LAST_QUESTION)
    oop_progress = get_paticular_progress(progress, oop_amount, qt.OOP_FIRST_QUESTION, qt.OOP_LAST_QUESTION)
    pep_progress = get_paticular_progress(progress, pep_amount, qt.PEP8_FIRST_QUESTION, qt.PEP8_LAST_QUESTION)
    structures_progress = get_paticular_progress(progress, structures_amount, qt.STRUCTURES_FIRST_QUESTION, qt.STRUCTURES_LAST_QUESTION)
    alghorimts_progress = get_paticular_progress(progress, alghorimts_amount, qt.ALGHORITMS_FIRST_QUESTION, qt.ALGHORITMS_LAST_QUESTION)
    git_progress = get_paticular_progress(progress, git_amount, qt.GIT_FIRST_QUESTION, qt.GIT_LAST_QUESTION)
    sql_progress = get_paticular_progress(progress, sql_amount, qt.SQL_FIRST_QUESTION, qt.SQL_LAST_QUESTION)
...
def get_paticular_progress(user_progress: dict, amount_kind: int, from_: int, to_: int) -> float:
    return round(
        len(
            [is_right for question_number, is_right in user_progress.items()
             if question_number in range(from_, to_) and is_right]) / amount_kind, 1
             )
